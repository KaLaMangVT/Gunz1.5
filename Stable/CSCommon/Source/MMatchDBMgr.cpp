...
TCHAR g_szDB_GET_ACHIEVEMENT_BYRequire[] = _T("{CALL spGetAchievementsByRequire ('%d','%d','%d')}");

TCHAR g_szDB_InsertAchievement[] = _T("{CALL spInsertAchievement ('%d','%d','%d')}");
...
bool MMatchDBMgr::FindAchievements(int CID, int nType, int nID)
{
	...
	CString strSQL;
	strSQL.Format(g_szDB_GET_ACHIEVEMENT_BYRequire, CID, nType, nID);
...
bool MMatchDBMgr::InsertAchievements(int CID, int nType, int nID)
{
	///TODO: get this correct
	_STATUS_DB_START;
	...
	CString strSQL;
	strSQL.Format(g_szDB_InsertAchievement, CID, nID, nType);
	...
	try
	{
		rs.Open(strSQL, CRecordset::dynaset, CRecordset::appendOnly);
