...
void MMatchStage::OnGameKill(const MUID& uidAttacker, const MUID& uidVictim)
{
	...
	if (MGetAchievements()->checkRequirements(player, GetStageSetting()->GetGameType(), player->GetKillCount(), node))
	{
		...
		*(MTD_Achievement*)MGetBlobArrayElement(achievementBlob, 0) = achievement;
		...
		MGetMatchServer()->GetDBMgr()->InsertAchievements(player->GetCharInfo()->m_nCID, node->type, node->id);
	}
}
